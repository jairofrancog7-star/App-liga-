...
function installMenus(){
  ...
  Object.entries(menus).forEach(([label,items])=>{
    ...
    const drop=document.createElement('div');drop.className='lj-dropdown'+(label==='Más'?' lj-wide':'');
    items.forEach(([text,r])=>{const x=document.createElement('button');x.type='button';x.textContent=text;x.dataset.ljRoute=r;drop.appendChild(x)});
    wrap.appendChild(drop);
    btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();document.querySelectorAll('.lj-menu-wrap.open').forEach(w=>w!==wrap&&w.classList.remove('open'));wrap.classList.toggle('open')});
  });
}
const dirSections=[
...
function storeMarkup(){return `<div class="lj-directory" data-lj-special="club-store"><div class="ds-utility"><div class="ds-wrap"><span><b style="color:#fff">LIGA</b> · Tienda de clubes</span><button data-lj-route="home">Cerrar ✕</button></div></div><div class="lj-directory-main" style="grid-template-columns:1fr"><main class="lj-directory-right"><h2>Tienda (clubes)</h2><p style="color:#b8cfda;font-size:12px;max-width:700px">Sección preparada para uniformes, artículos oficiales y productos de los equipos participantes. Los botones son de demostración hasta conectar inventario y pagos.</p><div class="lj-dir-grid" style="margin-top:30px">${['Juventino','Franco Tavera','Pozos','Cuenda','América','Rincón del Carmen','San Pedro','Morales','La Luz','Valencia','Real Juventino','Atlético Rosas'].map(n=>`<button class="lj-dir-item"><span class="lj-dir-icon">👕</span>${n}</button>`).join('')}</div></main></div></div>`}
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-lj-route]');if(b){e.preventDefault();document.querySelectorAll('.lj-menu-wrap.open').forEach(w=>w.classList.remove('open'));go(b.dataset.ljRoute);return}const auth=e.target.closest('[data-demo-auth]');if(auth){e.preventDefault();alert('Esta pantalla ya está diseñada. Para iniciar sesión de verdad falta conectar el proveedor de autenticación elegido.');}});
document.addEventListener('click',e=>{if(!e.target.closest('.lj-menu-wrap'))document.querySelectorAll('.lj-menu-wrap.open').forEach(w=>w.classList.remove('open'))});
