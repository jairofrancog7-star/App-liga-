...
function modalHtml(s){
  return '<div class="v144-modal"><button class="v144-backdrop" data-close></button><section><header><b>Fuente de transmisión</b><button data-close>×</button></header><label><span>Nombre del medio</span><input data-name value="'+esc(s.source.name||'Facebook / transmisión externa')+'" placeholder="Talacha Deportes"></label><label><span>Enlace del en vivo</span><input data-url value="'+esc(s.source.url||DEFAULT_SOURCE)+'"></label><label><span>Feed automático JSON (opcional)</span><input data-feed value="'+esc(s.source.feedUrl||'')+'" placeholder="https://.../live.json"></label><p>Facebook no permite que una página externa lea directamente su video/audio por seguridad. La detección automática usa el micrófono del dispositivo; un feed JSON opcional puede sincronizar eventos cada 12 segundos.</p><button class="v144-save" data-save>Guardar fuente</button></section></div>';
}
